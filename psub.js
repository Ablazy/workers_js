// This JS implementation replaces privacy information before subscription conversion
...
function replaceHysteria2(link, replacements) {
  ...
  const passwd = regexMatch[1];
  const server = regexMatch[2];
  const randomPassword = generateRandomStr(12);
  const randomDomain = generateRandomStr(12) + ".com";
  replacements[randomPassword] = passwd;
  replacements[randomDomain] = server;
  return link.replace(passwd, randomPassword).replace(server, randomDomain);
}
